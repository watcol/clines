...
impl Tile {
    pub fn new<T: AsRef<[u8]>>(chr_rom: T, index: u16) -> Self {
...
#[derive(Clone, Debug)]
pub struct ColoredTile([[Rgb; 8]; 8]);

impl ColoredTile {
    pub fn new(tile: &Tile, pallete: &[u8; 4]) -> Self {
        let mut res = [[Rgb::default(); 8]; 8];
        for y in 0..8 {
            for x in 0..8 {
                res[y as usize][x as usize] = COLORS[pallete[tile.get(x, y) as usize] as usize];
            }
        }
        Self(res)
    }
